...
Write-Host "3. With no gamepad connected, do not move the mouse or press keys, and wait at least 90 seconds."
Write-Host "4. Confirm the monitor turns off or Windows enters sleep according to the selected Windows setting."
Write-Host "5. Repeat with real DualSense/Pico2W, Steam Controller, and third-party gamepads connected but untouched."
Write-Host "6. Confirm connected-but-untouched gamepads do not block display-off or sleep."
Write-Host "7. Wake the screen, press a real guide/PS input, and confirm Bloss guide handling resumes without keeping the display awake permanently."
Write-Host "8. Put Bloss in tray, wait again, and confirm the same display-off or sleep behavior."
Write-Host "9. If it does not turn off, run this script again and compare powercfg /requests output."
Write-Host "10. If powercfg /requests or /waketimers says administrator rights are required, rerun only the snapshot from PowerShell as Administrator."
Write-Host ""
Write-Host "Manual gamepad-only active-use check"
Write-Host "1. Keep Windows display-off timeout at 1 minute."
Write-Host "2. Run test.exe and do not touch the mouse or keyboard."
Write-Host "3. Use only Steam Controller, DualSense/Pico2W, or another gamepad for more than 70 seconds."
Write-Host "4. Confirm real gamepad input delays display-off like keyboard/mouse, but mere connection does not."
Write-Host "5. Press the configured guide/notification button and confirm the battery guide still opens."
Write-Host "6. In another PowerShell window, this command can wait for the guide-button result:"
Write-Host "   powershell -ExecutionPolicy Bypass -File `".\scripts\show-guide-button-events.ps1`" -Wait -TimeoutSeconds 90 -RequirePopup"
Write-Host "7. To watch Windows idle time and Bloss gamepad activity together, use:"
Write-Host "   powershell -ExecutionPolicy Bypass -File `".\scripts\show-gamepad-idle-activity.ps1`" -TimeoutSeconds 90 -RequireActivity"
Write-Host "8. If automatic battery alerts are being tested, keep using only the controller and confirm alerts still appear when a selected threshold is crossed."
