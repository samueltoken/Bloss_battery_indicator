...
foreach ($check in @(
    ...
    @{ Path = $mainWindowTestsPath; Needle = 'ShouldShowReleaseNotes("1.1.0", "1.1.1", forceEveryRun: false)'; Message = "Release notes tests do not cover v1.1.0 update behavior." },
    @{ Path = $mainWindowTestsPath; Needle = 'ShouldShowReleaseNotes(" 1.1.1\r\n", "1.1.1", forceEveryRun: false)'; Message = "Release notes tests do not cover normalized seen-version behavior." },
    @{ Path = $mainWindowTestsPath; Needle = 'ShouldShowReleaseNotes("1.1.1", "", forceEveryRun: false)'; Message = "Release notes tests do not cover blank release-version suppression." },
    @{ Path = $mainWindowTestsPath; Needle = 'ShouldShowReleaseNotes("1.1.1", "", forceEveryRun: true)'; Message = "Release notes tests do not cover test.exe force-every-run behavior." },
    @{ Path = $previewArtifactTestsPath; Needle = 'new ReleaseNotesWindow("1.1.1")'; Message = "Release notes preview image is not rendered with the current release version." }
)) {
