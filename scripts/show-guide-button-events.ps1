...
function Get-GuideButtonEvents {
    ...
    Get-Content -LiteralPath $logPath -Tail $Count | ForEach-Object {
        ...
        (-not $CustomTriggerCheck -or $isCustomTriggerSignal) -and
        ($SteamPowerOffCheck -or $CustomTriggerCheck -or $Device -eq "Any" -or $_.Device -eq $Device) -and
        (-not $PressedOnly -or $SteamPowerOffCheck -or $CustomTriggerCheck -or $_.Event -in @("pressed", "popup_shown", "popup_missing_device", "tray_fallback")) -and
        (-not $RequirePopup -or $SteamPowerOffCheck -or $CustomTriggerCheck -or $_.Event -in @("popup_shown", "popup_missing_device", "tray_fallback")) -and
        ($Diagnostics -or $_.Event -notin @("service_started", "discovery_no_endpoints"))
    }
}

function Select-GuideButtonEventPublicFields {
    process {
        [pscustomobject]@{
            Time = $_.Time
            Event = $_.Event
            Device = $_.Device
            Detail = if ($_.Event -like "custom_trigger_*") { $_.Name } else { "" }
            Message = $_.Message
        }
    }
}
...
if ($Wait) {
    ...
    $target = if ($CustomTriggerCheck) { "custom notification-button" } elseif ($SteamPowerOffCheck) { "Steam Controller power-off" } elseif ($Device -eq "Any") { "guide-button" } else { $Device }
    $goal = if ($CustomTriggerCheck) { "capture/toast/suppression event" } elseif ($SteamPowerOffCheck) { "popup/suppression event" } elseif ($RequirePopup) { "popup result" } else { "press/popup event" }
    Write-Host "Waiting for $target $goal for $TimeoutSeconds seconds..."

    while ([DateTimeOffset]::Now -lt $deadline) {
        $events = Get-GuideButtonEvents -Count 80 | Where-Object {
            ($SteamPowerOffCheck -or $CustomTriggerCheck -or $_.Event -in @("pressed", "popup_shown", "popup_missing_device", "tray_fallback")) -and
            ([DateTimeOffset]$_.Time) -ge $startedAt
        }

        if ($events) {
            $events | Select-GuideButtonEventPublicFields | Format-Table -AutoSize -Wrap
            exit 0
        }
    ...
    } | Select-GuideButtonEventPublicFields | Format-Table -AutoSize -Wrap
    exit 1
}

Get-GuideButtonEvents -Count $Tail | Select-GuideButtonEventPublicFields | Format-Table -AutoSize -Wrap
